...
class ChatPage(BasePage):
    ...
    def _json_to_plot(self, json_dict: dict[str, Any] | None) -> gr.update:
        if json_dict:
    ...
    def on_register_events(self) -> None:
        # This method contains extensive event registration logic
        # that would be too large to include in full here
        pass
    ...
    def submit_msg(
        ...
        conv_id: str | None,
        conv_name: str | None,
        first_selector_choices: list[tuple[str, str]],
        request: gr.Request,
    ) -> list[Any]:
    ...
    def get_recommendations(
        self, first_selector_choices: list[tuple[str, str]], file_ids: list[str]
    ) -> str:
        first_selector_choices_map = {
    ...
    def toggle_delete(self, conv_id: str) -> tuple[gr.update, gr.update]:
        if conv_id:
            return gr.update(visible=False), gr.update(visible=True)
        else:
            return gr.update(visible=True), gr.update(visible=False)

    def on_set_public_conversation(self, is_public: bool, convo_id: str) -> None:
        if not convo_id:
    ...
    def persist_data_source(
        ...
        plot_history: list[dict[str, Any] | None],
    ...
    def reasoning_changed(self, reasoning_type: str) -> str:
    ...
    def is_liked(self, convo_id: str, liked: gr.LikeData) -> None:
        with Session(engine) as session:
            statement = select(Conversation).where(Conversation.id == convo_id)
            result = session.exec(statement).one()
    ...
    def message_selected(
        self,
        retrieval_history: list[str],
        plot_history: list[dict[str, Any] | None],
        msg: gr.SelectData,
    ) -> tuple[str | gr.update, dict[str, Any] | None]:
        index = msg.index[0]
        try:
    ...
    def suggest_chat_conv(
        ...
        chat_history: list[tuple[str, str | None]],
        use_suggestion: bool,
    ) -> tuple[gr.update, list[list[str]] | gr.update]:
        target_language = (
            session_language
            if session_language not in (DEFAULT_SETTING, None)
